...
int main(int argc, char **argv){
  ...
  if(data_gpu == NULL)
    printf("Data table allocation failed\n");
  printf("3\n");
  for(i = 0; i < n; i++){
  ...
  */
  ...
  cudaMallocPitch((void **)&data_gpu, NULL, n * sizeof(unsigned long), 2 * sizeof(unsigned long));
  ...
  cudaMemcpy2D((void *)&data_gpu, n*sizeof(unsigned long),(const void *)& data, n*sizeof(unsigned long), n*sizeof(unsigned long), 2*sizeof(unsigned long), cudaMemcpyHostToDevice);
